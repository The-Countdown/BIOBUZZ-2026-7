...
@Configurable // Panels
public class EighteenClose extends OpMode {
    private TelemetryManager panelsTelemetry; // Panels Telemetry instance
    public Follower follower; // Pedro Pathing follower instance
    private RobotContainer robotContainer;
    private int pathState; // Current autonomous path state (state machine)
    private EighteenClose.Paths paths; // Paths defined in the Paths class
    ...
    public void init() {
        ...
        paths = new EighteenClose.Paths(follower); // Build paths
        holding = false;
        actionRun = false;
    ...
    public void start(){
        ...
        pathTimer.reset();
        setPathState(0);
        Status.currentPose = Status.alliance == Constants.Game.ALLIANCE.RED ? new Pose2D(DistanceUnit.INCH, 124-72, 124-72, AngleUnit.DEGREES, HelperFunctions.normalizeAngle(218)) : HelperFunctions.mirror90(new Pose2D(DistanceUnit.INCH, 124-72, 124-72, AngleUnit.DEGREES, HelperFunctions.normalizeAngle(218)), DistanceUnit.INCH, AngleUnit.DEGREES);
    }
    ...
    private static class Paths {
        ...
        public Pose
                ...
                RED_GATE_COLLECT = new Pose(132.000, 64, Math.toRadians(25)),
    ...
    public void autonomousPathUpdate() {
        ...
        switch (pathState) {
            ...
            case 3:
                if (!follower.isBusy()) {
                    follower.followPath(paths.TM_SC, true);
                    incrementPathState();
                }
                break;
            case 4:
            ...
            case 6:
                if (!follower.isBusy()) {
                    follower.followPath(paths.SC_GC, false);
                    incrementPathState();
                }
                break;
            case 7:
            ...
            case 16:
                if (!follower.isBusy()) {
                    follower.followPath(paths.SC_TL, false);
                    intake.run();
                    incrementPathState();
                }
                break;
            case 17:
                if (!follower.isBusy()) {
                    endOfIntake.run();
                    follower.followPath(paths.TL_SC, true);
                    incrementPathState();
                }
                break;
            case 18:
